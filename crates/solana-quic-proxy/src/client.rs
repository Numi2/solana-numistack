...
use quinn::rustls::{pki_types::CertificateDer, ClientConfig as RustlsClientConfig, RootCertStore};
...
impl QuicRpcClient {
    ...
    async fn connection(&self) -> Result<Connection, ProxyError> {
        ...
        let connection = connecting.await.map_err(ProxyError::Connection)?;
    ...
    async fn request_inner(
        ...
        send.write_all(&len).await.map_err(ProxyError::Write)?;
        send.write_all(payload).await.map_err(ProxyError::Write)?;
        send.finish()
